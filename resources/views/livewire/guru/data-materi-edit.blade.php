                                    ...
                                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                                        <div class="p-6 bg-white border-b border-gray-200">
                                            <form wire:submit.prevent>
                                                ...
                                                @php
                                                    $fm = substr($old_file_materi, 14);
                                                @endphp
                                                    <button name="delete" id="delete" class="btn btn-danger btn-sm" 
                                                    data-toggle="modal" data-target="#mdlDelMat">
                                                        Hapus File Sebelumnya
                                                    </button>
                                                    <span>Berikut adalah file yang sebelumnya diunggah: <a href="{{ route('downloadFileMatLama', ['foldname' => $old_file_materi]) }}">{{ $fm }}</a></span>
                                                @endif
                                                @elseif ($file_materi != null)
                                                <label for="">File Materi</label> <br>
                                                @php
                                                    $fm = $file_materi->getClientOriginalName();
                                                @endphp
                                                <button name="delete" id="delete" class="btn btn-danger btn-sm"
                                                wire:click="file_null">
                                                    Hapus File Sebelumnya
                                                </button><span>&emsp;{{ $fm }}</span>
                                                @endif
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            @php
                                $fm = substr($old_file_materi, 14);
                            @endphp
                            Apakah Anda yakin ingin <strong>MENGHAPUS</strong> file materi <strong>{{ $fm }}</strong>?
                            @endif
                        </div>
